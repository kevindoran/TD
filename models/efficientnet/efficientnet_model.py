...
class MBConvBlock(tf.keras.layers.Layer):
  ...
  def __init__(self, block_args, global_params, hparams):
  ...
  def dropout_1x1conv(self, x, out_channels, training, scope):
      with tf.variable_scope(scope):
          in_channels = x.shape[self._channel_axis]
          filter_size = 1
          kernel = tf.get_variable(
              'DW', # Name used in pruning.
              [filter_size, filter_size, in_channels, out_channels], tf.float32,
              initializer=conv_kernel_initializer)
          kernel = dropouts.ramping_targeted_weight_random(
              kernel, self._hparams, training)
          data_format = "NHWC" if self._data_format == "channels_last" else "NCHW"
          conv = tf.nn.conv2d(x, kernel, [1,1], padding='SAME',
                              data_format=data_format)
          conv = tf.identity(conv, name='post_dropout')
          return conv

  def dropout_dw_conv(self, x, filter_size, strides, training):
      with tf.variable_scope('depthwise_conv2d'):
          in_channels = x.shape[self._channel_axis]
          depth_multiplier = 1
          kernel = tf.get_variable(
              'DW', [filter_size, filter_size, in_channels, depth_multiplier],
              tf.float32, initializer=conv_kernel_initializer)
          kernel = dropouts.ramping_targeted_weight_random(
              kernel, self._hparams, training)
          if self._data_format == 'channels_last':
              ...
              assert self._data_format == 'channels_first'
              data_format = 'NCHW'
              strides = (1, 1) + tuple(strides)
          conv = tf.nn.depthwise_conv2d(x, kernel, strides=strides,
                                        padding='SAME', data_format=data_format)
          return conv

  # def se_reduce(self, x, out_channels, training):
  #     se_reduced = model_utils.conv(
  #         x,
  #         filter_size=1,
  ...
  #         bias = tf.get_variable('bias', [out_channels], initializer='tensor')
  ...
  def call(self, inputs, training=True):
    ...
    if self._block_args.expand_ratio != 1:
      ...
      x = self._relu_fn(
              self._bn0(
                  self.dropout_1x1conv(inputs, filters, training=training,
                                       scope='expand'),
                  training=training))
    else:
      x = inputs
    ...
    x = self._relu_fn(
            self._bn1(
                self.dropout_dw_conv(x, kernel_size,
                                     strides, training=training),
                training=training))
    ...
    x = self._bn2(
      self.dropout_1x1conv(x, self._block_args.output_filters,
                           training=training, scope='project'),
      training=training)
    ...
    if self._block_args.id_skip:
      if all(
        ...
        # Updated
        # Drop-connect is added for each convolution block.
...
class Model(tf.keras.Model):
  ...
  def __init__(self, blocks_args=None, global_params=None, hparams=None):
    """Initializes an `Model` instance.

    Args:
      blocks_args: A list of BlockArgs to construct block modules.
      global_params: GlobalParams, a set of global parameters. This parameter
        is being commandeered to take in hparams as used in targeted_dropout
        repo.
  ...
  def _build(self):
    ...
    for block_args in self._blocks_args:
      ...
      self._blocks.append(conv_block(block_args, self._global_params, hparams=self._hparams))
